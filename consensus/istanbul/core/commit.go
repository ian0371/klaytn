...
func (c *core) handleCommit(msg *message, src istanbul.Validator) error {
	...
	// TODO-VRANK: consider verifying commit
	if isVrankTargetCommit(commit, src) {
		vrankCommitArrivalTimeMap[src.Address()] = timeSincePreprepare()
	}
...
func (c *core) acceptCommit(msg *message, src istanbul.Validator) error {
	...
	if c.current.Commits.Size() == 1 {
		vrankFirstCommitArrivalTimeGauge.Update(int64(timeSincePreprepare()))
	}
