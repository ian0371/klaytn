...
func TestGovParamSet_Get(t *testing.T) {
	...
	assert.False(t, ok)
	assert.Nil(t, v)
	assert.Nil(t, p.MustGet(CommitteeSize))
}
...
func TestGovParamSet_Nominal(t *testing.T) {
	...
	assert.Equal(t, c.Governance.GovernanceGen, p.GovernanceGenStr())
	assert.Equal(t, c.Governance.GovParamsContract, p.GovParamsContract())
...
func TestGovParamSet_NominalPanic(t *testing.T) {
	...
	// Calling nominal getter for non-existing key cause panic
	p.CommitteeSize()
}
