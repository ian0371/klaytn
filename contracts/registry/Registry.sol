...
interface IRegistry {
    // events
    event SetAddress(bytes32 indexed name, address addr);

    // mutators

    /**
     * Writes a record.
     * @param name  The record name left-padded to 32-bytes (zeros added before name)
...
/**
 * @dev Contract to store global system contract addresses.
 *
 * Inherits Ownable for _owner, onlyOwner
 * Inherits IRegistry for its functions
 */
contract Registry is Ownable, IRegistry {
    ...
    // mutator impls
    function setAddress(bytes32 name, address addr)
        external
        override
        onlyOwner
    {
        _records[name] = addr;
        emit SetAddress(name, addr);
    }

    // getter impls
    function getAddress(bytes32 name)
        external
        view
        override
        returns (address)
